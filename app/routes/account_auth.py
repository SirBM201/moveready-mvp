    ...
    return jsonify(
        {
            ...
            "request_limits": {
                ...
                "max_per_email": AUTH_OTP_MAX_REQUESTS_PER_EMAIL_WINDOW,
                "max_per_ip": AUTH_OTP_MAX_REQUESTS_PER_IP_WINDOW,
            },
        }
    )
...
def request_code():
    ...
    try:
        rate_limit = _request_rate_limit(email)
    except Exception:
        return jsonify({"ok": False, "error": "otp_rate_limit_check_unavailable"}), 503
    if rate_limit:
        response = jsonify({"ok": False, **rate_limit})
    ...
    if not delivery_ok and not dev_allowed:
        try:
            get_supabase().table("relocation_auth_login_codes").update({"status": "expired"}).eq("id", (stored or {}).get("id")).execute()
        except Exception:
            pass
        return jsonify(
            {
    ...
    result: Dict[str, Any] = {
        ...
        "email": email,
        "expires_at": _iso(expires_at),
        "delivery_status": delivery.get("status") if delivery_ok else "development_code",
        "delivery_provider": delivery.get("provider") if delivery_ok else "development",
    }
    if dev_allowed:
...
def verify_code():
    ...
    if not email or not code or not CODE_RE.fullmatch(code):
        return jsonify({"ok": False, "error": "valid_email_and_six_digit_code_required"}), 400
    ...
    if not login_code:
        return jsonify({"ok": False, "error": "invalid_or_expired_code"}), 400
...
# Keep compatibility with account.py and other protected routes that reuse the auth helpers.
