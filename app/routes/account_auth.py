...
from app.services.email_delivery import deliver_login_code
...
def request_code():
    ...
    result: Dict[str, Any] = {
        ...
        "email": email,
        "expires_at": _iso(expires_at),
        "delivery_status": delivery.get("status") or ("sent" if delivery_ok else "email_delivery_not_configured"),
        "delivery_provider": delivery.get("provider"),
    }
    if _dev_code_allowed():
        result["dev_code"] = code
    status_code = 200 if delivery_ok else 202
...
# Keep compatibility with account.py, which reuses the auth helper functions.
