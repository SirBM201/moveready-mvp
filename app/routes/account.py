...
def _profile_status_priority(row: Dict[str, Any]) -> int:
    ...
    Supabase returns rows in created_at desc order. Python sorting is stable, so
    sorting only by active/non-active keeps newest profiles first inside each
    group and moves the chosen account profile to the top.
    """
