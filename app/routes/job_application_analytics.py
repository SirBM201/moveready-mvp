...
from app.services.job_application_performance_intelligence import (
...
def application_analytics_attribution():
    ...
    if dimension not in ALLOWED_DIMENSIONS:
        return jsonify(
            {
                "ok": False,
                "error": "unsupported_attribution_dimension",
...
def application_performance_dimension(dimension):
    ...
    if dimension not in SUPPORTED_DIMENSIONS:
        return jsonify(
            {
                "ok": False,
                "error": "unsupported_performance_dimension",
                "allowed": list(SUPPORTED_DIMENSIONS),
            }
        ), 400
    return jsonify(
        {"ok": True, **dimension_intelligence(load_account_portfolio(email), dimension)}
    )
