...
from app.services.job_application_performance_intelligence import CONTRACT_VERSION as PERFORMANCE_VERSION, SUPPORTED_DIMENSIONS, dimension_intelligence, performance_overview
...
def application_analytics_attribution():
    ...
    return jsonify({"ok":True,"contract_version":CONTRACT_VERSION,"dimension":dimension,"breakdown":attribution_breakdown(items,dimension),"portfolio_count":len(items)})

@bp.get("/application-analytics/funnel")
def application_analytics_funnel():
    email,error=_account()
    if error:return error
    result=metrics(load_account_portfolio(email))
    return jsonify({"ok":True,"contract_version":CONTRACT_VERSION,"funnel":result["funnel"],"rates":result["rates"],"terminal_outcomes":result["terminal_outcomes"],"applications_tracked":result["applications_tracked"]})
...
def application_performance():
    ...
    if error:return error
    items=load_account_portfolio(email)
    return jsonify({"ok":True,**performance_overview(items)})
