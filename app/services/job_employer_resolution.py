...
from app.services.job_employer_intelligence import employer_identity,same_employer

CONTRACT_VERSION="b19.11.2-v1"

def resolve_employer(vacancy:Mapping[str,Any],candidate:Mapping[str,Any])->dict[str,Any]:
    vacancy_identity=employer_identity({"name":vacancy.get("company_name") or vacancy.get("company") or vacancy.get("employer"),"domain":vacancy.get("company_domain") or vacancy.get("employer_domain") or vacancy.get("company_url"),"country":vacancy.get("country") or vacancy.get("location_country")})
    comparison=same_employer(vacancy_identity,candidate)
