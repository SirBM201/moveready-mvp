...
-- Account-private, service-role-only persistence. Idempotent.
begin;
...
create table if not exists public.relocation_job_search_campaigns (
  ...
  target_occupations jsonb not null default '[]'::jsonb,
  target_employers jsonb not null default '[]'::jsonb,
  work_authorized_countries jsonb not null default '[]'::jsonb,
  sponsorship_required boolean not null default false,
  relocation_support_preferred boolean not null default false,
  search_intensity text not null default 'standard' check (search_intensity in ('light','standard','intensive')),
  notes text,
  contract_version text not null default 'b19.10.1-v1',
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);

create index if not exists relocation_job_search_campaigns_email_status_idx
  on public.relocation_job_search_campaigns(email,status,updated_at desc);
...
create index if not exists relocation_job_search_campaign_vacancies_email_idx
  on public.relocation_job_search_campaign_vacancies(email,campaign_id,created_at desc);
create index if not exists relocation_job_search_campaign_vacancies_job_idx
  on public.relocation_job_search_campaign_vacancies(job_id);
